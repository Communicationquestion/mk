...
int addmodule::Add_Module::creta_files(std::string _file_name,
                                       std::string _type) {

  set_paths(_type);
  m_paths.print_paths();

  m_paths.create_file<fs::path>(
      m_paths.get_path<fs::path>("module_root_folder_path"), "");
  set_module_cmake_txt(std::move(_file_name), std::move(_type));
  m_paths.create_file<fs::path>(
      m_paths.get_path<fs::path>("camke_txt_file_path"), module_cmake_txt,
      false);
  ...
  m_paths.create_file<fs::path>(m_paths.get_path<fs::path>("main_path"), "",
                                false);

  return 0;
}
int addmodule::Add_Module::set_module_cmake_txt(std::string _file_name,
                                                std::string _type) {
  std::string str{};
  ...
  std::string target_include_directories =
...
void addmodule::Add_Module::set_paths(std::string _type) {
  ...
  template_path::Template_Path<fs::path, std::string>::pathtype
      module_root_folder_path = std::string(m_modeule_folder);
  m_paths.set_path("module_root_folder_path", module_root_folder_path);
  if ("project" == _type) {

    template_path::Template_Path<fs::path, std::string>::pathtype
        module_root_folder_path = std::string(m_modeule_folder+"/"+m_modeule_folder);
        m_paths.set_path("module_root_folder_path", module_root_folder_path);
  }

  fs::path tmp_camke_txt_file_path =
      m_paths.get_path<fs::path>("module_root_folder_path") / "CMakeLists.txt";
  template_path::Template_Path<fs::path, std::string>::pathtype
      camke_txt_file_path = tmp_camke_txt_file_path;
  m_paths.set_path("camke_txt_file_path", camke_txt_file_path);

  fs::path tmp_module_include_path =
      m_paths.get_path<fs::path>("module_root_folder_path") / "include" /
      m_modeule_folder;
  template_path::Template_Path<fs::path, std::string>::pathtype
      module_include_path = tmp_module_include_path;
  m_paths.set_path("module_include_path", module_include_path);

  fs::path tmp_src_path =
      m_paths.get_path<fs::path>("module_root_folder_path") / "src";
  template_path::Template_Path<fs::path, std::string>::pathtype src_path =
      tmp_src_path;
  ...
  template_path::Template_Path<fs::path, std::string>::pathtype main_path =
      tmp_main_cpp_path;
