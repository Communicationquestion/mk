...
int new_project::NewProject::set_project_cmake_txt(const std::string& _file_name) {
    ...
    project_cmake_txt =
        cmake_minimum_required + set_cpp_standard + set_standard_required + set_extensions + project + add_subdirectory;
    return 0;
}

void new_project::NewProject::set_paths() {
    template_path::Template_Path<fs::path, std::string>::pathtype project_root_folder_path =
