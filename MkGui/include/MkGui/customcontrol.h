...
struct Control<InputText, T...> {
	static void Draw(const std::string& label, T*... values) {
		...
		std::cout << "Sum of values for " << label << ":" << sum << std::endl;
		if constexpr (sizeof...(T) > 0) {
			((std::is_same_v<T, std::string> ? ImGui::InputText((label + "##" + std::to_string(reinterpret_cast<std::uintptr_t>(values))).c_str(), &(*values)) : 0), ...);
		}
	}
};
...
class CustomControl {
	...
	void set_data() {
		std::apply([thsi](auto&&... args) {Control<ControlType, DataTypes...>::set_data(args...);}, data_);
	}
	private:
	std::string label_;
